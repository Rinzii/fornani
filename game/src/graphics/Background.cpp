...
void Background::render(automa::ServiceProvider& svc, sf::RenderWindow& win, sf::Vector2<float> cam) {
	...
	for (auto& layer : layers) {
		...
		if (locked_vertically()) { layer.final_position.y = std::clamp(layer.final_position.y, std::min(static_cast<float>(-scroll_pane.y + svc.window->current_size().y), -1 + epsilon), 0.f); }
		if (locked_horizontally()) { layer.final_position.x = std::clamp(layer.final_position.x, std::min(static_cast<float>(-scroll_pane.x + svc.window->current_size().x), -1 + epsilon), 0.f); }
		auto ctr{0};
